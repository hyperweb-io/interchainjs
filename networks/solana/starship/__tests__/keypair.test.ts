...
describe('Keypair', () => {
  ...
  test('should restore from base58 secret keys', () => {
    const keypair = Keypair.generate();
    const base58Secret = bs58.encode(keypair.secretKey);

    const restored = Keypair.fromBase58(base58Secret);

    expect(restored.publicKey.toString()).toBe(keypair.publicKey.toString());
  });
