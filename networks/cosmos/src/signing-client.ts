...
export class SigningClient {
  ...
  async searchTx(query: SearchTxQuery | string): Promise<SearchTxResponse> {
    let rawQuery: string;
    let prove = false;
    let page = 1;
    let perPage = 100;
    ...
    const data = await fetch(
      `${this.endpoint.url}/tx_search?${params.toString()}`
    );
    const json = await data.json();
    return json['result'];
  }
  ...
  async searchBlock(query: SearchBlockQuery | string): Promise<SearchBlockResponse> {
    let rawQuery: string;
    let page = 1;
    let perPage = 100;
    let orderBy: 'asc' | 'desc' = 'asc';
    ...
    const data = await fetch(
      `${this.endpoint.url}/block_search?${params.toString()}`
    );
    const json = await data.json();
    return json['result'];
  }
