// networks/cosmos/starship/__tests__/queryclient/query-client.test.ts
...
describe('Cosmos Query Client', () => {
  ...
  beforeAll(async () => {
    queryClient = createCosmosQueryClient(OSMOSIS_RPC, {
      timeout: 15000,
      headers: {
  ...
  describe('Connection Management', () => {
    test('should connect successfully', () => {
      ...
      expect(queryClient.endpoint).toBe(OSMOSIS_RPC);
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle network timeouts', async () => {
      const shortTimeoutClient = createCosmosQueryClient(OSMOSIS_RPC, {
...
describe('Client Factory', () => {
  test('should create query client with factory', () => {
    const client = CosmosClientFactory.createQueryClient('https://rpc.osmosis.zone/', {
      protocolVersion: ProtocolVersion.COMET_38,
      timeout: 10000
    });
    
    expect(client).toBeDefined();
    expect(client.endpoint).toBe('https://rpc.osmosis.zone/');
