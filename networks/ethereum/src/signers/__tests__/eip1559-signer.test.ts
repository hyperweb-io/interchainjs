...
import { EIP1559EthereumSigner } from '../eip1559-signer';
import { EIP1559TransactionSignArgs } from '../types';
...
describe('EIP1559EthereumSigner', () => {
  let signer: EIP1559EthereumSigner;
  ...
  beforeEach(async () => {
    ...
    // Create signer
    signer = new EIP1559EthereumSigner(wallet, { queryClient });
  });

  describe('Basic functionality', () => {
    it('should create signer successfully', () => {
      expect(signer).toBeDefined();
      expect(signer).toBeInstanceOf(EIP1559EthereumSigner);
    });
  ...
  describe('Transaction signing', () => {
    it('should sign an EIP-1559 transaction', async () => {
      const txArgs: EIP1559TransactionSignArgs = {
        transaction: {
      ...
      expect(signedTx.rawTx).toMatch(/^0x02[a-fA-F0-9]+$/); // EIP-1559 prefix
    ...
    it('should sign transaction with specific nonce', async () => {
      ...
      expect(signedTx).toBeDefined();
      expect(signedTx.txHash).toMatch(/^0x[a-fA-F0-9]{64}$/);
      expect(signedTx.rawTx).toMatch(/^0x02[a-fA-F0-9]+$/);
    });
    ...
    it('should throw error for missing maxFeePerGas', async () => {
      const txArgs: any = {
        transaction: {
          to: '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6',
          value: '0x0',
      ...
      await expect(signer.sign(txArgs)).rejects.toThrow('maxFeePerGas is required');
    });

    it('should throw error for missing maxPriorityFeePerGas', async () => {
      const txArgs: any = {
        transaction: {
          to: '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6',
          value: '0x0',
      ...
      await expect(signer.sign(txArgs)).rejects.toThrow('maxPriorityFeePerGas is required');
    });
    ...
    it('should throw error for wrong transaction type', async () => {
      const txArgs: any = {
        transaction: {
          to: '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6',
          value: '0x0',
          ...
          maxPriorityFeePerGas: '0x77359400',
          gas: '0x5208',
          type: '0x0', // Wrong type - will be treated as legacy and require gasPrice
          data: '0x'
        }
      };

      await expect(signer.sign(txArgs)).rejects.toThrow('gasPrice is required for legacy transactions');
    });
  });

  describe('Helper methods', () => {
    it('should sign with auto fees', async () => {
      const transaction = {
        to: '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6',
      ...
      const signedTx = await signer.signWithAutoFees(transaction);
      expect(signedTx).toBeDefined();
      expect(signedTx.txHash).toMatch(/^0x[a-fA-F0-9]{64}$/);
      expect(signedTx.rawTx).toMatch(/^0x02[a-fA-F0-9]+$/);
    });
