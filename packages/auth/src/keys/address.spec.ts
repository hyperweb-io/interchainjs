...
import { createCosmosConfig, createEthereumConfig, createInjectiveConfig } from '../config/builders';
...
describe('Address', () => {
  ...
  describe('fromPublicKey', () => {
    ...
    it('should create Injective address from public key', async () => {
      const hdPath = HDPath.cosmos(0, 0, 0);
      const walletConfig = createInjectiveConfig();
      const privateKeys = await PrivateKey.fromMnemonic(testMnemonic, [hdPath], walletConfig.privateKeyConfig);
      const publicKey = privateKeys[0].toPublicKey();
      ...
      expect(address.config.strategy).toBe('injective');
    });
  ...
  describe('fromString', () => {
    ...
    it('should parse Injective address', () => {
      ...
      expect(() => {
        Address.fromString(addressString, { strategy: 'injective' });
      }).toThrow(/Invalid injective address format/);
    });
  ...
  describe('checksum validation', () => {
    it('should validate Ethereum checksum', () => {
      // Note: Our placeholder implementation doesn't validate checksums yet
      // This test documents the expected behavior for the real implementation
      ...
      // Our placeholder returns false for all non-standard addresses
